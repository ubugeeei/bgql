...
export function deferred<TParent, TArgs, TContext, TResult>(
  resolver: ResolverFn<TParent, TArgs, TContext, TResult>
): ResolverFn<TParent, TArgs, TContext, Promise<TResult>> {
  return (parent, args, context, info) => {
    return Promise.resolve().then(() => resolver(parent, args, context, info));
  };
}
 ...
 */
export function batched<TParent, TArgs, TContext, TResult, TKey = string>(
