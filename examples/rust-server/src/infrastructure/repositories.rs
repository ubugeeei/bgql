...
impl InMemoryUserRepository {
    pub fn with_seed_data() -> Self {
        ...
        for (id, name, email, bio) in [
            ("user_1", "Alice Johnson", "alice@example.com", Some("Software engineer")),
            ("user_2", "Bob Smith", "bob@example.com", None),
            ("user_3", "Carol Williams", "carol@example.com", Some("Tech writer")),
        ] {
            let user = User {
                id: UserId::new(id),
                name: name.to_string(),
...
impl UserRepository for InMemoryUserRepository {
    ...
    async fn find_all(&self, pagination: Pagination) -> Connection<User> {
        ...
        let edges: Vec<Edge<User>> = items
            .into_iter()
            .map(|u| Edge { cursor: u.id.0.clone(), node: u })
            .collect();
...
impl InMemoryPostRepository {
    pub fn with_seed_data() -> Self {
        ...
        for (id, author_id, title, content, status) in [
            ("post_1", "user_1", "Introduction to BGQL", "Better GraphQL is...", PostStatus::Published),
            ("post_2", "user_1", "Schema-First Development", "In schema-first...", PostStatus::Published),
            ("post_3", "user_2", "DataLoaders Explained", "DataLoaders solve...", PostStatus::Published),
            ("post_4", "user_1", "Draft: Typed Errors", "Work in progress...", PostStatus::Draft),
            ("post_5", "user_3", "Rust and GraphQL", "Building GraphQL servers...", PostStatus::Published),
        ] {
            let post = Post {
                id: PostId::new(id),
        ...
        Self { posts: RwLock::new(posts), next_id: RwLock::new(6) }
    }
}
...
impl PostRepository for InMemoryPostRepository {
    ...
    async fn find_all(&self, filter: PostFilter, pagination: Pagination) -> Connection<Post> {
        ...
        let mut all: Vec<_> = posts.values()
            .filter(|p| {
                filter.status.map(|s| p.status == s).unwrap_or(true) &&
                filter.author_id.as_ref().map(|a| &p.author_id == a).unwrap_or(true)
            })
            .cloned()
            .collect();
        ...
        let edges: Vec<Edge<Post>> = items
            .into_iter()
            .map(|p| Edge { cursor: p.id.0.clone(), node: p })
            .collect();
    ...
    async fn update(&self, id: &PostId, update: PostUpdate) -> Option<Post> {
        ...
        if let Some(post) = posts.get_mut(id) {
            if let Some(title) = update.title { post.title = title; }
            if let Some(content) = update.content { post.content = content; }
            if let Some(status) = update.status { post.status = status; }
...
impl InMemoryCommentRepository {
    pub fn new() -> Self {
        Self { comments: RwLock::new(Vec::new()), next_id: RwLock::new(1) }
    }
}
