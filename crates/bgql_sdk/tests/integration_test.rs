...
/// Test BgqlError construction
#[test]
fn test_bgql_error() {
    let error = BgqlError::new("CUSTOM_ERROR", "Something went wrong");
    assert_eq!(error.code, "CUSTOM_ERROR");
    assert_eq!(error.message, "Something went wrong");

    let network_err = BgqlError::network("Connection refused");
    assert_eq!(network_err.code, "NETWORK_ERROR");

    let parse_err = BgqlError::parse("Invalid syntax");
    assert_eq!(parse_err.code, "PARSE_ERROR");

    let timeout_err = BgqlError::timeout();
    assert_eq!(timeout_err.code, "TIMEOUT");

    let not_found = BgqlError::not_found("User");
